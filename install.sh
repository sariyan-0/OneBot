...
_ensure_shared_data_mount() {
  local shared_data="/data"
  local install_data="$INSTALL_DIR/data"

  mkdir -p "$install_data"

  if [[ -L "$shared_data" ]]; then
    local target
    target="$(readlink -f "$shared_data" 2>/dev/null || true)"
    if [[ "$target" == "$install_data" ]]; then
      return 0
    fi
    rm -f "$shared_data"
  elif [[ -e "$shared_data" ]]; then
    if [[ -d "$shared_data" ]]; then
      _warn "/data already exists as a directory; leaving it in place."
      return 0
    fi
    _warn "/data already exists and is not a directory or symlink; leaving it in place."
    return 0
  fi

  ln -s "$install_data" "$shared_data"
  _ok "Shared data path ready: $shared_data -> $install_data"
}
